...
To demonstrate the behavior of the correlation functions for a more realistic system, a simulation for an particle between two finite plates was implemented. To prevent the particle disappearing from the plates the system is limited in all directions with reflecting boundaries. 
\begin{figure}[H]
\centering
...
\end{tikzpicture}
\caption{Schematic structure of the finite plate system -- The plates which the particle can adsorb are marked as $A$ and $B$ }
\end{figure}
Above plate $A$ and below plate $B$ there are reflecting boundaries as well. The system can also be described as a slit or an pore in which the particle can adsorb. The only possible trajectory which goes from the left side to the right side of the system goes through this slit.
