...
\subsubsection{Single surface}
For a single surface $A$, one particle that is adsorbed at time $t=0$ can basically keep adsorbed until time $t$ or desorb $n$--times before it is adsorbed again, if it should be adsorbed at time $t$. 
\begin{figure}[H]
\centering
...
\label{fig:ways}
\end{figure}
As illustrated in the figure above $C_{AA}(t)$ as the sum of different paths. The distribution $P(t)$ is the Probability for an adsorbed ion to desorb at time $t$ for the first time. $Q_A(t)$ is the survival distribution for an ion to be adsorbed on the surface $A$ over the time span $t=0$ to $t$. Note that this equals the probability that ion desorbes at some time $> t$.It is given by:
\begin{align}
